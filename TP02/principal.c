...
int main() {
	...
	maquina(memoriaInstrucoes, RAM, cache1, cache2, cache3);
...
void maquina(Instrucao *memoriaInstrucoes, BlocoMemoria *RAM, BlocoMemoria *cache1, BlocoMemoria *cache2, BlocoMemoria *cache3) {
	...
	while (opcode != HALT) {
		...
		if (opcode != -1) {
			...
			BlocoMemoria dadoMemoriaEnd1 = buscarNasMemoriasAssociativo(umaInstrucao.end1, RAM, cache1, cache2, cache3);
			dadoMemoriaEnd1.tempoChamado = tempoExecucao;
			for (int i = 0; i < TAMANHO_CACHE1; i++) {
				if (dadoMemoriaEnd1.endBloco == cache1[i].endBloco) {
					cache1[i] = dadoMemoriaEnd1;
			...
			BlocoMemoria dadoMemoriaEnd2 = buscarNasMemoriasAssociativo(umaInstrucao.end2, RAM, cache1, cache2, cache3);
			dadoMemoriaEnd2.tempoChamado = tempoExecucao;
			for (int i = 0; i < TAMANHO_CACHE1; i++) {
				if (dadoMemoriaEnd2.endBloco == cache1[i].endBloco) {
					cache1[i] = dadoMemoriaEnd2;
			...
			BlocoMemoria dadoMemoriaEnd3 = buscarNasMemoriasAssociativo(umaInstrucao.end3, RAM, cache1, cache2, cache3);
			dadoMemoriaEnd3.tempoChamado = tempoExecucao;
			for (int i = 0; i < TAMANHO_CACHE1; i++) {
				if (dadoMemoriaEnd3.endBloco == cache1[i].endBloco) {
					cache1[i] = dadoMemoriaEnd3;
